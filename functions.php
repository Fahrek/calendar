			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						//For Add Event
						echo '<a href="javascript:;" onclick="addEvent(\''.$currentDate.'\');">añadir evento</a>';
						echo '</div></div>';
				...
				$.ajax({
					...
					success:function(msg){
						if(msg == 'ok'){
							...
							alert('Algún problema a ocurrido, por favor, inténtelo de nuevo.');
						}
					}
				});
			});
...
function getEvents($date = ''){
	...
	if($result->num_rows > 0){
		$eventListHTML = '<h2>Eventos el '.date("l, d M Y",strtotime($date)).'</h2>';
		$eventListHTML .= '<ul>';
